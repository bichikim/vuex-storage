(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('lodash'), require('cookie')) :
    typeof define === 'function' && define.amd ? define(['exports', 'lodash', 'cookie'], factory) :
    (global = global || self, factory(global.VuexStorage = {}, global._, global.Cookie));
}(this, function (exports, lodash, cookie) { 'use strict';
    ...
    ***************************************************************************** */
    ...
    var Cookies = /** @class */ (function () {
        ...
        Cookies.prototype._updateCookie = function (options) {
            ...
            if (_cookie) {
                this._cookies = lodash.merge(this._cookies, cookie.parse(_cookie.toString(), options));
            }
        };
    ...
    function storeExceptOrOnly(_state, except, only) {
        var state = lodash.cloneDeep(_state);
        var clonedState = {};
        if (!only && !except) {
            return clonedState;
        }
        if (only) {
            clonedState = lodash.pick(state, only);
        }
        else {
            clonedState = state;
        }
        if (except) {
            clonedState = lodash.omit(clonedState, except);
        }
        return clonedState;
    }
    var VuexStorage = /** @class */ (function () {
        function VuexStorage(options) {
            ...
            var getStateFilter = function (dynamicFilter) {
                return {
                    cookie: lodash.get(_this._store.state, dynamicFilter.cookie || ''),
                    session: lodash.get(_this._store.state, dynamicFilter.session || ''),
                    local: lodash.get(_this._store.state, dynamicFilter.local || ''),
                };
            };
            ...
            var mergeState = function (state) {
                ...
                var _state = state;
                var originalState = lodash.cloneDeep(store.state);
                if (storageFirst) {
                    _state = lodash.merge(originalState, state);
                }
                else {
                    _state = lodash.merge(state, originalState);
                }
                if (strict) {
            ...
            this.restoreFilter = function (context) {
                ...
                mergeState(lodash.merge(localState, cookieState));
            };
            this.restore = function (context) {
                ...
                mergeState(lodash.merge(sessionState, localState, cookieState));
            };
    ...
    exports.DEFAULT_MUTATION_NAME = DEFAULT_MUTATION_NAME;
    exports.DEFAULT_SAVE_METHOD = DEFAULT_SAVE_METHOD;
    exports.FILTERS_KEY = FILTERS_KEY;
